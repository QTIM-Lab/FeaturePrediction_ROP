...
class RetinaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            gest_age = sample_df['gestationalAgeWeeks'].iloc[0]
            ex_preterm = 1 if gest_age < 26. else 0 # 42 to 58
            label_to_int = {'No': 0, 'Pre-Plus': 1, 'Plus': 2}
            plus = label_to_int[sample_df['Golden Reading Plus'].iloc[0]]
            if self.transform:
                image = self.transform(image)
            sample = {'image': image, 'age': ex_preterm, 'plus': plus, 'img': img_name}
        except:
            sample = None
        return sample
...
def prepare(action, data_dir, csv_file, save_file=None):
    ...
    # class_weights = torch.FloatTensor([1./2332, 1./3246, 1./87]).cuda()
    # class_sample_count = [2332, 3246, 87] # dataset has 10 class-1 samples, 1 class-2 samples, etc.
    # weights = 1. / torch.Tensor(class_sample_count)
    # samplr = sampler.WeightedRandomSampler(weights, batch_size)
